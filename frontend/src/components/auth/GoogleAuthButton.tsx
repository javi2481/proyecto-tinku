...
interface Props {
  label?: string;
  next?: string; // path al que volver después del login
}
...
export function GoogleAuthButton({ label = 'Continuar con Google', next = '/dashboard' }: Props) {
  ...
  const onClick = async () => {
    ...
    try {
      ...
      // redirectTo basado en window.location.origin — respeta el host actual
      // (preview.emergentagent.com vs localhost). Passthrough ?next= para flows
      // que quieran volver a un destino específico.
      ...
      // Si ok, el browser ya se fue a Google — no reset loading.
    } catch {
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="w-full h-12 rounded-2xl bg-white border-2 border-tinku-ink/15 hover:border-tinku-ink/30 text-tinku-ink font-semibold flex items-center justify-center gap-3 transition-colors disabled:opacity-60 disabled:cursor-wait"
      >
      ...
      {err && (
        <p role="alert" data-testid="google-auth-error" className="text-xs text-tinku-warn text-center">
          {err}
        </p>
      )}
    </div>
